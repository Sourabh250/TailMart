    ...
    try {
        ...
        if (data) {
            data.forEach(element => {
                ...
                <div class="flex flex-col items-center p-2 shadow-lg h-[600px]">
                <img src="${element.image}" class="w-1/2 md:w-1/4 h-1/2" alt="Image Of Product">
                <div class="px-6 py-4 flex flex-col h-1/2">
                <h1 class="font-bold text-xl mb-2 text-stone-700">${element.title}</h1>
                <p class="text-gray-700 overflow-hidden">${element.description}</p>
                <div class="pt-4 pb-2 text-gray-700 mt-auto overflow-hidden"><span class="inline-block bg-gray-300 rounded-full px-3 my-3 font-semibold hover:bg-sky-500 cursor-pointer hover:text-white">Category: ${element.category}</span>
